...
class ImdbQuery
  @@ADDRESS_POST = '127.0.0.1:27018'
  def self.query_moviedb_by_year(year)
    array = Array.new
    client = Mongo::Client.new([@@ADDRESS_POST], :database => 'movieactor', :monitoring => false)
  ...
  def self.get_movie_cluster_by_year()
  ...
  def self.query_actordb_by_year(year)
    year = year.to_s
    array = Array.new
    client = Mongo::Client.new([@@ADDRESS_POST], :database => 'movieactor')
  ...
  def self.get_actor_cluster_by_year()
